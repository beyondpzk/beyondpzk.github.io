...
## 四、延迟指标：P50 / P95 / P99 是什么意思？

在 Orin 部署里，「模型推理一次需要多少毫秒」通常不是单一数字，而是一组**分位延迟（percentile latency）**。最常见的是 P50、P95、P99：

| 指标 | 含义 | 工程意义 |
|---|---|---|
| **P50（中位数）** | 50% 的推理请求在这个时间内完成 | 典型用户体验，反映日常运行的「平均」水平 |
| **P95** | 95% 的推理请求在这个时间内完成 | 接近最坏情况，决定系统能否满足实时 deadline |
| **P99** | 99% 的推理请求在这个时间内完成 | 长尾延迟，影响系统稳定性和抖动容忍度 |

为什么不能只报平均延迟？因为 Orin 上存在很多会导致延迟突发的因素：

- **CPU/GPU 内存竞争**：共享内存架构下，其他进程占用带宽会让推理 stall；
- **JetPack 后台任务**：日志、守护进程、thermal throttling 会带来偶发干扰；
- **动态 shape / 变长输入**：比如 LLM 的生成长度不同，每帧计算量差异很大；
- **CPU↔GPU 数据拷贝**：某些帧如果触发 fallback 或内存交换，延迟会瞬间飙升。

举例：某目标检测模型在 Orin NX 上测得：
...
如果只报平均 8 ms，会误以为可以轻松跑 100 FPS；但 P99 的 45 ms 意味着每 100 帧就有 1 帧接近 2 倍控制周期，在闭环机器人控制中可能直接造成一次失控。

### 在 Orin 上怎么测？

推荐使用 `trtexec` 或自己写循环推理脚本，记录每帧耗时，再算分位值：

```bash
trtexec --loadEngine=model.engine --avgRuns=1000
```

`trtexec` 的输出里会自带 `percentile(90%)`、`percentile(95%)`、`percentile(99%)` 等字段。正式统计前建议先 warm-up 几百帧，把 cache、frequency、thermal 稳定下来。

如果是自己写脚本，可以用 Python 的 `numpy.percentile`：

```python
import numpy as np
latencies = [...]  # 每帧推理耗时，单位 ms
...
- **实时控制任务**（机器人避障、无人机姿态）：看 **P99**，确保最坏情况下也不超 deadline；
- **视频流推理**（检测、跟踪）：看 **P95**，允许极少数帧掉链子，但整体上要流畅；
- **离线/低优先级任务**：看 **P50** 或平均延迟即可。

一句话：**P50 决定「快不快」，P95/P99 决定「稳不稳」。** 在 Orin 这种资源受限、共享内存、易受系统干扰的平台上，尾部延迟往往比平均延迟更能决定产品体验。
...
> **重要澄清**：GPU 支持的算子范围比 DLA 大得多。**DLA 能跑的算子，GPU 一定能跑；但 GPU 能跑的算子，DLA 不一定能跑**。所以 DLA 是 GPU 的一个**子集**，而不是超集。
>
> 这也意味着：不存在「这个算子不能跑 DLA，但跑 GPU 会更快」的反面情况——DLA 快不快取决于它能不能跑、以及模型结构是否适合它。
>
...
### Fallback 到底是什么？涉及数据拷贝吗？

**Fallback（回退）** 是指 TensorRT 发现某个算子无法在当前指定的加速器（如 DLA）上执行，于是自动把它交给另一个加速器（通常是 GPU，严重时是 CPU）去跑。

在桌面 GPU 上，fallback 往往意味着数据要从 CPU 内存拷贝到 GPU 显存。但 **Orin 是统一内存架构**，CPU、GPU、DLA 共用同一块 LPDDR5，所以**通常不需要跨物理内存拷贝**。

不过 fallback 仍然有开销：

| 开销类型 | 说明 |
|---|---|
| **执行引擎切换** | 从 DLA 切换到 GPU，需要同步两个硬件的调度队列 |
| **数据格式转换** | DLA 喜欢的格式（如 `kDLA_HWC4`）和 GPU 喜欢的格式（如 `kCHW`）可能不同，TensorRT 可能插入 reshape/transpose/format 转换 |
| **缓存同步** | DLA 写出的结果在 DRAM 中，GPU 去读前需要保证缓存一致性 |
| **权重复制/重排** | 同一层的权重，DLA 和 GPU 需要的布局可能不同 |
| **kernel launch 开销** | 频繁 fallback 会导致大量小 kernel 启动 |

#### 一个典型例子

```text
Conv (DLA) → Reshape (DLA 不支持) → Softmax (DLA)
```

TensorRT 会这样处理：

1. `Conv` 在 DLA 跑，输出写到共享内存；
2. `Reshape` 不支持 DLA，**fallback 到 GPU**；
3. TensorRT 可能做一次格式转换，让 GPU 能读 DLA 的输出；
4. GPU 跑 `Reshape`；
5. 结果再转回 DLA 需要的格式；
6. `Softmax` 回到 DLA。

本来 `Conv` 和 `Softmax` 都能跑 DLA，中间夹了一个 `Reshape`，导致**两次 DLA↔GPU 切换 + 两次格式转换**。如果模型里这种切换很频繁，延迟会明显变差。

#### GPU fallback 到 CPU 会更重

如果某个算子 GPU 也不支持，只能 fallback 到 CPU，开销通常更大：

- CPU 访问 GPU 分配的内存可能走慢路径；
- 可能需要把 tensor 换到 CPU 可高效访问的内存区域；
- CPU 算子并行度低，延迟高。

所以 Orin 上的开销排序大致是：

```text
纯 GPU ≈ 纯 DLA < DLA↔GPU fallback << GPU↔CPU fallback
```
...
## 十、什么时候在一个 Orin 模块内做切分？

前面说的「多设备部署」是把模型拆到**多块 Orin 板子**上。而「单节点内切分」是指**同一块 Orin 内部**，把模型的不同部分放到 GPU / CPU / DLA 上执行。

适合做这种切分的场景：

### 1. GPU 内存不够，但模型又能拆成 GPU/CPU 两段

比如一个 7B LLM 在 Orin NX 16GB 上：

- 把前面大部分 decoder layer 放 GPU；
- 把最后几层或 KV cache 换出到 CPU 执行。

代价是延迟增加，但能让模型跑起来。

### 2. 想让 DLA 承担连续大段 CNN，释放 GPU

如果模型 backbone 是 ResNet/YOLO 风格，连续几十层都是卷积+BN+ReLU：

- backbone 放 DLA；
- head / 后处理放 GPU；
- 这样 GPU 可以空闲出来跑其他模型或任务。

### 3. 模型里有 CPU 才支持的算子

某些特殊算子（如复杂索引、动态控制流、某些 PyTorch 自定义 op）在 TensorRT GPU backend 里也没有实现，只能放到 CPU 上跑。这时其他层仍留在 GPU，形成单节点内切分。
...
- backbone 用 DLA（低功耗）；
- 后处理和跟踪逻辑用 CPU；
- GPU 只在需要高精度/复杂任务时才启用。

### 5. 多模型并发时，给 GPU 减负

同一块 Orin 上要跑感知、VLM、LLM 多个模型。把其中一部分 CNN 感知模型 offload 到 DLA，可以给 VLM/LLM 留出更多 GPU 内存和算力。

### 单节点切分的代价

虽然 Orin 的 CPU 和 GPU 共享内存，但**执行域切换**仍然有开销：

- GPU kernel 结束 → CPU 同步 → CPU 执行 → 再调 GPU；
- DLA 输出 → GPU 继续计算，涉及缓存同步和内存映射；
- 频繁切换会吃掉收益，甚至让 latency 变差。

所以单节点内切分的关键是：**切出来的每一段要足够长、足够连续，让切换开销被计算收益覆盖**。
