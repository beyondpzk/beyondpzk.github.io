...
**注意"BEV"出现在两处，别混**：
- 输入侧的 **BEV 坐标目标**（`(x,y)` → MLP → 伪 token）：只有 Point-Goal 用；另外四个任务的 goal 都是文本，走 text tokenizer；
- 输出侧的 **BEV 航点**（5 个 `(x,y,θ)`）：五个任务共用，是统一的动作输出表示（见 §3.4）。

这正是"异构输入、同构输出"：输入目标五花八门（坐标 / 文本 / 类别 / POI / 人物描述），输出统一成 BEV 航点。
...
#### 3.4.3 几个实现细节：读出位置、输入、推理步数

**是最后一个 token 进 Flow Matching 吗？论文没写那么细。** Qwen-RobotNav 明确写 `final hidden state E_A ∈ R^d`（最后 token 的隐藏向量）→ MLP；ABot-N0 只写"用 Brain 的 latent context 条件化 Action Expert"，没有写明具体取哪个位置（最后 token / 池化 / 特殊 token）。方向上和 Qwen-RobotNav 一样（LLM 隐状态 → 条件化动作头），但读出位置是留白，复现要看代码。

**Flow Matching 的输入**：两个东西——(1) 条件 `c`（Brain 处理视觉 + 语言目标 + 视觉历史 + 推理任务后的隐状态）；(2) 当前噪声航点状态 `W_t` 和时间 `t`，即向量场 `v_θ(W_t, t | c)`。它建模的是"高斯噪声 → 5 个 BEV 航点（15 维）"的连续变换。

**推理不是一步到位**：从高斯噪声采样 `W_0`，沿 `dW_t/dt = v_θ(W_t, t | c)` 从 `t=0` 积分到 `t=1`，用 Euler 或更精确的 ODE solver 多步求解（步数是超参），不是 MLP 那种一次前向。

**与 Qwen-RobotNav 的对比**：

| | Qwen-RobotNav | ABot-N0 |
|---|---|---|
| 动作头 | 4 层 MLP（确定性回归） | Flow Matching（生成式） |
| 输入 | 最后 token 的 final hidden state（明确写出） | Brain 的 latent context（未写位置） |
| 输出 | 24 维（8 航点） | 15 维（5 航点） |
| 推理 | 一步前向 | 多步 ODE 积分 |
| 多模态 | 否（平均掉多个合理轨迹） | 是（可采样不同绕行模式） |
...
**如何构建 / 维护**：论文明确给出的是“动态维护（maintenance-in-the-loop）”机制，而非“从零建图”的完整算法。

- 每一次任务执行，无论成功还是失败，观测、轨迹、交互结果都会作为 **evidence 写回拓扑图**：成功 → `Confirmed`，失败 → `Excluded`；
- 环境变化（临时封路、施工围挡、走廊拥堵）→ 自适应更新图结构或边权重，避免“反复撞同一个过时记忆”；
- 可乐例子：零食架没可乐 → `零食架 | 可乐 = Excluded`；售货机找到 → `售货机 | 可乐 = Confirmed`。下次 Planner 查询自动跳过零食架。

所以 Topo-Memory 的“构建”是**用出来的**——不是一次性建好，而是每次任务都在往图上沉淀新节点、更新旧节点可达性。

**留白**：论文没有给出初始图（Block/Road/Function/Object 四层节点如何从原始观测提取、边如何连、坐标如何标定、AMAP 路由如何融进四层）的详细算法，只给了框架 + 更新循环。合理推断是室外粗粒度骨架来自 AMAP 全局路由、Object/POI 与 Function 层靠视觉观测 + 结果写回逐步填充，但这是推断，不是论文原文。

**室内外定位与"先建图"的不对称**：户外是清楚的——RTK-GNSS 绝对定位 + AMAP 全局路由 + LiDAR 局部占用图，闭环闭合。但室内有硬伤：**RTK 在室内失效**，论文只提了"腿部里程计 + 局部建图管线"（相对/局部定位，用于避障和走眼前这段路），没有交代室内**全局定位**（我在几楼哪个房间）从哪来；Topo-Memory 的 Block/Road 层需要这种跨区域定位，室内这块是空缺。同时"Map-as-Memory 边用边建"意味着**冷启动第一次进一栋楼时图是空的**，机器人要么先探索一遍、要么得有先验地图，论文没给冷启动方案。合理推断是室内粗骨架靠高德室内地图 seed、Object/POI 层靠使用写回，但这是推断。对照：GuideNav 用 teach + 拓扑 VPR，FARM 用 Stage 1 遥操作探索建图，ABot-N0 户外讲清楚、室内"先建图 + 全局定位"是留白。

**三种"图"与三种"定位"**（导航任务几乎没有"没图"这回事，差别在形态和来源）：

| 图形态 | 例子 | 对应任务 | 对应定位 |
|---|---|---|---|
| 度量图 | `(x,y)` 坐标、占用栅格 / SLAM | PointNav / Point-Goal | 度量定位（`relative_goal = global_goal - current_pose`） |
| 拓扑图 | Topo-Memory、GuideNav 关键帧链 | 跨区域 / teach-repeat | 拓扑定位（我在哪个节点） |
| 语言路线图 | "穿过客厅，经过厨房，在红沙发旁停下" | VLN | 路标重识别（走到哪个里程碑） |

所以"提前建图 + 度量定位"这个强需求，只对"给坐标的度量型任务"成立；VLN 不是"没图"，而是"图由用户现给（语言路线图）+ 路标定位"；ObjNav 则是"语义先验 + 在线记忆"。差别是图的形态和来源，不是有没有图。
...
#### 5.7.11 为什么有了端侧 LLM 认知大脑，还要云端 VLM Planner？

这两个"大脑"管的是两个时间尺度和粒度，不是重复：

| | 端侧 LLM（Cognitive Brain） | 云端 VLM（Agentic Planner） |
|---|---|---|
| 频率 | 2Hz 连续 | 事件触发（子任务开始/结束/失败） |
| 粒度 | 局部：这一步怎么绕、怎么走 | 全局：任务怎么拆、顺序、失败重规划 |
| 输出 | 条件化 Action Expert → 5 航点 | 子任务序列 `G`、反思结果 `(r, f)` |
| 上下文 | 当前观测 + 短期视觉历史 | Topo-Memory + 全 episode 记忆 + 指令 |
| 推理方式 | 任务条件化（双头 If-Else，非显式 CoT） | 显式 CoT（policy / critic 两种 prompt） |

**为什么不能合成一个**：端侧 4B LLM 塞不下 Topo-Memory + 长程上下文，也没被训练成通用奖励模型做进度判断/反思；而且它是任务条件化设计（不走 verbose CoT）。云端 VLM 是文本输出 + 有网络延迟，做不了 2Hz 连续航点生成，连续 `(x,y,θ)` 多模态轨迹要靠 Flow Matching。

**配合例子（可乐）**：
1. 云 Planner（policy）收到"我要可乐" → CoT："先零食架 A、没有再去售货机 B" → 输出 `g₁=Point-Goal→A`、`g₂=Object-Goal→搜可乐`；
2. 端 LLM + Flow Matching 执行 `g₁`、`g₂`（只负责走好每一步，不思考"没可乐怎么办"）；
3. 云 Self-Reflector（critic，同一 VLM）看结果 → `(r=False, "零食架没可乐")`，写回 Topo-Memory；
4. 云 Planner 带反馈重规划 → `g₃=Point-Goal→B`、`g₄=Object-Goal→售货机`；
5. 端 LLM 执行 → 找到可乐 → Self-Reflector 确认。

数学上就是 `P(W | I, M^L, M^S) = P(G | ...) × ∏ P(W^j | g_j, ...)`：前者是云 VLM 的 Agentic Planning，后者是端 LLM + Flow Matching 的 ABot-N0 执行。高德长距离同理：云 VLM 把"方向意图"精修成"前方 8m 人行道内侧"的局部子目标，端 LLM 只负责走好这 8m。一句话：**"战略脑"和"战术脑"分层——一个低频想得远，一个高频走得稳。**

#### 5.7.12 整个系统到底有多少个模型（端侧 + 云端）

运行时共 **3 个逻辑模型，拆到子模型是 6 个**：

| 位置 | 逻辑模型 | 内部子模型 | 职责 | 频率 |
|---|---|---|---|---|
| 端侧 Orin NX | ABot-N0 Actor | SigLIP-B/16 ViT（93M）、Qwen3-4B LLM、Flow Matching | 看 + 想 + 出航点 | 2Hz |
| 端侧 Orin NX | Neural Controller | VelFlow、RL refiner | 航点 + LiDAR → 速度 | >10Hz |
| 云端 RTX 4090 | VLAC（Planner/Self-Reflector 同一 VLM） | 1 个 VLM（通用奖励模型） | 拆任务 + 反思纠错 | 事件触发 |

（π3、MoGe、Wan2.1-I2V、Qwen-VL-Max、Gemini-3 Pro 等是离线造数据 / 当教师的模型，不算运行时。）

**用"我要一罐可乐"走一遍配合**：
1. 云 VLAC（policy）CoT 拆任务 → `g₁=Point-Goal→零食架A`、`g₂=Object-Goal→搜可乐`；
2. 端 ABot-N0 执行 `g₁`：SigLIP ViT 编码三视角 → Qwen3-4B 推理 → Flow Matching 出 5 航点；
3. 端 Neural Controller：航点 + LiDAR 占用图 → VelFlow + RL refiner → 速度 `(v_x, v_y, v_yaw)` 驱动 Go2；
4. 端 ABot-N0 执行 `g₂` 搜可乐；
5. 云 VLAC（critic）看结果：没可乐 → `(r=False, "试售货机")`，写回 Topo-Memory；
6. 云 VLAC（policy）重规划 → `g₃`、`g₄`；
7~8. 端侧再执行 → 找到可乐 → 云 VLAC（critic）确认写回。

一句话：**1 个云 VLM 管"去哪/找什么/失败换哪"，1 个端侧 VLA 管"这一米怎么走"，1 个端侧控制器管"航点变脚步"。**

#### 5.7.13 与 Qwen-RobotNav 的评测集对比

**ABot-N0（7 个基准）**：

| 任务 | 基准 | 环境侧重 |
|---|---|---|
| Point-Goal 开环 | CityWalker | 户外（城市街景，网络视频） |
| Point-Goal 闭环 | SocNav | 户外（社会导航） |
| Instruction-Following | VLN-CE R2R-CE / RxR-CE | 室内 |
| Object-Goal | HM3D-OVON（开放词汇） | 室内 |
| POI-Goal | BridgeNav | 室外→室内过渡（POI 入口） |
| Person-Following | EVT-Bench | 室内 |

**Qwen-RobotNav（评测集 + 真实世界）**：

| 任务 | 基准 | 环境侧重 |
|---|---|---|
| Instruction-Following | VLN-CE R2R/RxR、VLNVerse、VLN-PE | 室内 |
| Object-Goal 闭词汇 | MP3D、HM3D v2 | 室内 |
| Object-Goal 开放词汇 | HM3D-OVON | 室内 |
| 主动视觉跟踪 | EVT-Bench（STT） | 室内 |
| 具身问答 EQA | HM-EQA、MT-EQA、EXPRESS-Bench | 室内 |
| 自动驾驶 | NAVSIM navtest、AlpaSim | 户外道路（车） |

**对比**：
- 三个共同：VLN-CE（R2R/RxR）、HM3D-OVON、EVT-Bench；
- ABot-N0 独有：CityWalker、SocNav、BridgeNav（户外行人 + 社会合规 + POI 最后几米）；
- Qwen-RobotNav 独有：VLNVerse、VLN-PE、MP3D ObjectNav、HM3D v2、三个 EQA、NAVSIM、AlpaSim。

一句话：高德在"户外行人 + 社会合规 + POI 最后几米"上有独有评测，Qwen 在"室内 VLN 全家桶 + EQA + 车端驾驶"上有独有评测；两者只在 VLN-CE、HM3D-OVON、EVT-Bench 三处交叠。

**补充：关于 Point-Goal 评测的不对称**——Qwen-RobotNav 训练了 984K 条 PointNav 样本（§4.1.2），PointNav 也是其四个 task mode 之一，但**论文评测里没有任何 PointNav benchmark 结果表**；它把 PointNav 定位为"训练用的移动原语 / 对照基线"（原文："PointNav also serves as a controlled baseline"），加上标准 Habitat PointNav 早已被 DD-PPO（2020）做到近乎满分、属于饱和任务，所以没作为成果任务对外展示。ABot-N0 则相反，有 CityWalker + SocNav 两个 Point-Goal 评测。即：**训练上两者都有 PointNav，评测上只有 ABot-N0 评了 Point-Goal。**
