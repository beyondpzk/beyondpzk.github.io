...
![NVIDIA Jetson Orin NX 模组（图片来源：NVIDIA 官网）](./2026-09-11-orin-nx-module.jpeg)

中间带 NVIDIA logo 的金属盖下就是 SoC 硅片（GA10B），四周的银色屏蔽罩下是 4 颗 LPDDR5 内存颗粒和供电电路（红色线圈为供电电感），底部金手指是 260-pin SO-DIMM 连接器——模组通过它插到载板上，所有相机、网口、USB 信号都从这里进出。
...
### 1.1 补充基础：CPU 是什么，为什么 CPU 也能装进 SoC

**CPU = Central Processing Unit，中央处理器**——负责**通用、串行**计算的核心：按顺序执行指令，做判断、跳转、调度。设计目标是"任何任务都能做，且单件事做得快"（低延迟）：复杂分支逻辑、操作系统、文件读写、网络协议栈都靠它。和 GPU 对比着记：

| | CPU | GPU |
|---|---|---|
| 核心数 | 几个到几十个，每个很强 | 成百上千个，每个很简单 |
| 擅长 | 复杂逻辑、分支判断、串行任务 | 海量简单重复的并行计算（矩阵乘、像素） |
| 类比 | 几个数学教授 | 一屋子小学生 |

**SoC = System on Chip，片上系统**——把一整台计算机需要的所有部件集成到**同一颗硅片**上。传统 PC 是"分立"的：CPU 一颗芯片、GPU 在显卡上、内存是插条、控制器在主板上，靠长走线互联；SoC 则把 CPU + GPU + 内存控制器 + ISP + DLA + 视频编解码器 + IO 控制器全部做进一颗指甲盖大的硅片（Orin NX 的 GA10B、你手机里的骁龙/苹果 A 系列都是 SoC）。

**为什么能装进去？两个原因：**

1. **它们本质上是同一种东西——晶体管组成的数字电路**。CPU、GPU、ISP 只是"电路设计图纸"不同。先进制程（Orin 用 8nm，约 170 亿个晶体管）让一颗小硅片装得下一整套系统。
2. **IP 核授权模式（行业分工）**：ARM 公司**不卖芯片，只卖 CPU 的设计图纸**（Cortex-A78AE 就是 ARM 设计的）。NVIDIA 买来 ARM 的 CPU 图纸，和自己设计的 GPU、DLA、ISP 图纸"拼积木"拼成一颗完整 SoC，交给三星/台积电代工生产。所以"CPU 在 SoC 里"不是把一颗现成的 CPU 芯片塞进去，而是**把 CPU 的设计和 GPU 的设计画在同一张版图、同一块硅片上**。

**好处**：部件间走线极短（统一内存、零拷贝、省掉 PCIe）、功耗低、体积小——所以手机、机器人、车载全是 SoC。**代价**：焊死不可升级、所有部件共享同一个功耗墙。

**为什么有了 GPU 还必须有 CPU**：GPU 自己开不了机——需要 CPU 先启动操作系统、加载驱动、准备好数据、再"发号施令"启动 kernel（16.2 节的 kernel launch 就是 CPU 在干活）。**CPU 是指挥官，GPU 是工厂**——Orin NX 那 6 核 Cortex-A78AE 干的就是这个，外加跑 Linux、ROS、相机驱动这些通用任务。

### 1.2 CUDA Core 在哪里：从模组到硅片到 SM 的层级

常有人问"SoC 里有没有 CUDA Core、CUDA Core 放在哪"——答案是：**就在 SoC 硅片的 GPU 区域里、SM 的内部**。完整层级：

```
Orin NX 模组（70×45 mm 电路板）
└── SoC 硅片 GA10B（指甲盖大，8nm，约 170 亿晶体管）
    ...
    ├── DLA × 2
    ├── ISP（图像信号处理器）
    ├── 内存控制器（摆在硅片边缘，贴近 LPDDR5 走线）
    └── PCIe / USB / CSI 等 IO 控制器
```
...
**LPDDR5（16 GB）**：层级最底，容量的保证、带宽的瓶颈。

#### 3.1.2 命中与未命中：延迟差多少

以 1 GHz 时钟粗略换算（1 周期 = 1 ns）：
...
每层"命中"就停，"未命中"（miss）就下沉一层取数据并回填。缓存层级有效的原因是**局部性原理**：刚用过的数据很可能再用（时间局部性），用过某地址很可能用相邻地址（空间局部性，一次取一整条 cache line）。

#### 3.1.3 为什么缓存救不了 LLM decode

这里有个反直觉但关键的推论：**层级缓存对 decode 几乎无效**。4B 模型 8 GB 权重，而 L2 只有 ~2 MB——权重总量是 L2 容量的 **4000 倍**，每步 decode 把所有权重读一遍、下一步再读一遍，L2 里根本留不住，命中率≈0。所以权重几乎 100% 从 LPDDR5 来，102.4 GB/s 成为绕不过去的硬地板（3.2 节的 78 ms）。

KV-cache（几百 MB）稍好：最热的一小段能留在 L2/L1 里，但主体仍然要回 LPDDR5。这就是为什么边缘平台优化 LLM 的重心是"少搬"（量化、小模型）而不是"缓存"。
...
### 3.4 有 L1、L2，为什么没有 L3

NVIDIA GPU 传统上就是 **L1 + L2 两级，没有 L3**——这不是偷工减料，是设计哲学不同：

- **CPU 靠缓存层级掩盖延迟**：CPU 只有几个到几十个核，同时运行的线程少，访问内存要等几百个周期、没活可干只能干等，所以需要 L1→L2→L3 层层兜底。
- **GPU 靠海量线程掩盖延迟**：每个 SM 同时驻留上千个线程，一组线程等内存时，调度器立刻切到另一组继续算——**用"并行度"代替"缓存深度"**。只要线程够多，内存延迟就被掩盖掉了，不需要 L3。
...
## 十八、KV-cache 到底提速多少？VLA 的 action chunk 下还有用吗

### 18.1 VLM 自回归推理：KV-cache 的定量收益

**算力维度：节省约等于平均序列长度的倍数。** 用本文的例子（prefill 210 token，decode 63 步）：

- 有 KV-cache：总计算 = 210（prefill）+ 63 × 1（decode 每步 1 个 token）= **273 个 token 的前向**
- 无 KV-cache：第 t 步要重跑全部 210+t 个 token，总计 Σ(210+t) = 63×210 + 63×62/2 ≈ **15,183 个 token 的前向**

**算力节省 ≈ 15,183 ÷ 273 ≈ 55 倍**。通用公式：(N·n + n²/2) ÷ (N+n)——生成越长（n 越大）节省越多，n=512 时约 330 倍。

**但墙上时间的提速远小于 55 倍**，这正是 Orin NX 的特殊之处：decode 是带宽瓶颈，每步 78 ms 的权重搬运无论如何省不掉。无 KV-cache 时每步相当于做一遍全序列 prefill，多出来的是重算 210+t 个 token 的计算时间：

| | 每步耗时 | 63 步合计 |
|---|---|---|
| 有 KV-cache | 78（搬运）+ 14（计算）≈ 92 ms | ≈ 5.8 s |
| 无 KV-cache | 78（搬运）+ 平均 ~50-90 ms（重算）≈ 150 ms | ≈ 9.5 s |

**墙上提速只有 ~1.6 倍**（生成越长倍数越大）。规律：**算力富裕的平台（桌面 GPU）上 KV-cache 接近满额提速；带宽瓶颈的平台（Orin NX）上提速被带宽地板压缩**——但省下的算力可以腾给别的模型（见第十四节多模型预算）。

代价也要记住：KV-cache 要占内存（4B 模型长序列可达 1-2 GB），从 16 GB 预算里扣。

### 18.2 VLA 的 action chunk（非自回归）下，KV-cache 还有用吗

**在动作维度上：没用。** Action chunk（ACT、π0 那种一次输出未来 30-50 步动作）是单次前向并行输出整个动作序列，没有逐 token 的 decode 循环——没有循环，就没有"重复计算历史"可省。
...
一句话总结：**KV-cache 省的是"相同上下文的重复计算"，不是"自回归"本身**——只要模型里存在"多步迭代、上下文不变"的结构（跨帧不变的指令、去噪循环里不变的观测），它就有用武之地；action chunk 只是把"动作"这一维从循环里拿掉了而已。

---

## 十九、非自回归 VLA 在 Orin NX 上的估算——终于轮到 TOPS 说话

VLA 里的 action chunk 路线（ACT、π0 类）每个控制周期只做**一次完整前向**（相当于一次 prefill），速度不由 decode 决定。估算方法和前面 decode 主导的场景有明显不同。
...
**第 1 步：内存（老规矩）**——权重 × 1.3 + 系统 4 GB ≤ 16 GB。

**第 2 步：单帧延迟 = 搬运时间与计算时间取大者（两者可部分重叠），再加 ViT 和动作头**：
...
### 19.3 三个典型 VLA 的估算（Orin NX 16GB）
