...
2. **切分到 CPU 内存**：把部分层放到 CPU 上执行，GPU 和 CPU 之间逐层传递；
...
- 或者把一个大模型切出部分层到 CPU，给其他模型让出 GPU 内存。
...
这是最轻量的「切分」。当 GPU 内存不足时，把部分层或 KV cache 放到 CPU 内存，需要时再搬回 GPU。
...
在 LLM 推理中，**offload 常用于长上下文**，把较早的 KV cache 换出到 CPU 内存或磁盘。
