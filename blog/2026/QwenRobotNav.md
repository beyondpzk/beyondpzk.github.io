...
一个常见的问题是：Qwen-RobotNav 本身能不能直接完成长距离导航，比如从当前位置走到几百米外的另一个房间或建筑物？

**严格来说，不能。** Qwen-RobotNav 是一个局部导航执行器，而不是全局路径规划器。

#### 为什么它本身不能独立完成长距离导航？

从模型设计层面看，Qwen-RobotNav 的输出始终定义在**以当前机器人位姿为原点的局部坐标系**中（`:279-291`）。它每次只预测未来 `K=8` 个航点，有效规划范围大约在 **5~10 米**。模型输入不包括全局地图、GPS 坐标或拓扑信息，因此它无法一次性规划从 A 到 B 的长程路径。

```
输入：局部观测 + 相对目标（文本指令或局部坐标）
输出：未来 8 个局部航点
有效范围：约 5~10 米
```

如果直接给它一个远距离全局目标，模型既不知道全局方向，也无法保证跨越大尺度空间后的累积定位精度。

#### 长距离导航如何被实现？

长距离能力来自**上层 Planner + 定位模块**的配合：
...
定位模块获取 current_pose
        ↓
relative_goal = global_goal - current_pose
        ↓
上层 Planner 拆分为 sub-goals（每段 5~10m）
        ↓
逐段调用 Qwen-RobotNav 输出局部航点
        ↓
执行并反馈，更新记忆，继续下一段
```

这与论文 `:829-846` 描述的双层 Agentic 架构一致：

| 层级 | 模型 | 职责 |
|---|---|---|
| 上层规划器 | Qwen3.6-Plus | 长程推理、目标分解、坐标转换、记忆更新 |
| 下层执行器 | Qwen-RobotNav | 局部视觉-语言-动作导航，输出 8 个航点 |
...
| 模型 | 自身有效范围 | 长距离依赖 |
|---|---|---|
| ABot-N0 | 5~15 米局部航点 | Agentic Planner + 高德地图 + Topo-Memory |
| Qwen-RobotNav | 5~10 米局部航点 | 上层 Qwen3.6-Plus Planner + 定位模块 + 证据笔记本 |

#### 长距离导航用 PointNav 还是 VLN？

一个常见的延伸问题是：长距离导航的每一段子目标，应该走 **PointNav**（给相对坐标）还是 **VLN**（给自然语言指令）？

**直觉上似乎只能走 VLN**，因为全局地图/高德给的目标通常是粗略的。但这个直觉需要修正：**不是“不能走 PointNav”，而是“不能直接拿粗略坐标当 PointNav 目标”。**

##### 为什么直接走 PointNav 会有问题？

以高德地图为例，参考 ABot-N0 博客 `:753-755` 的分析：

> 高德 API 返回的 waypoint 精度在米级（通常 1~5 米偏差），且设计目标是为人或车规划路线——它标注的是路口中心点，不区分人行道、马路和隔离带。直接喂给导航模型做 Point-Goal 会导致机器人走向马路中间而不是人行道。

所以 `粗略 waypoint → PointNav` 是有风险的。

##### 方案 A：Planner 局部精修 + PointNav

Planner 拿到粗略 waypoint 后，先做一次**视觉 grounding + 语义判断**，再生成精确的局部子目标：
...
Planner 局部目标精修
  输入：
    ...
    - Topo-Memory / 语义图层
  输出：
    - 局部子目标：前方 8m 处人行道内侧，相对位姿 (dx, dy, θ)
        ↓
Qwen-RobotNav PointNav 模式
```

精修后的 `(dx, dy, θ)` 是当前画面里可见、可达、社会合规的局部目标，精度足够走 PointNav。这个思路在 ABot-N0 博客 `:758-784` 中有详细展开。

##### 方案 B：语义转换 + VLN

如果觉得坐标精修链路太重，也可以直接把路由意图转成自然语言指令：

```
高德路由：往东北方向沿 XX 路走 200m → 路口右转
        ↓
VLN 指令：沿人行道往东北方向走，到第一个路口过斑马线右转
        ↓
Qwen-RobotNav VLN 模式
```

自然语言对坐标误差的容忍度更高——“走 200 米到路口”不依赖某个具体经纬度值。这也是 ABot-N0 博客 `:787-798` 中提到的“更好的方案”。

##### 两种方案如何选？

| 场景 | 推荐模式 | 原因 |
|---|---|---|
| 有精确局部坐标（如精修后、室内定位、已知路标点） | **PointNav** | 精度高、路径直接、可控性强 |
| 只有粗略方向意图或语义路标 | **VLN** | 对坐标误差更鲁棒，表达更自然 |
| 需要绕过复杂语义约束（如走人行道、避开施工区） | **VLN** | 语言能直接编码社会规范与意图 |
| 需要精确到达特定 POI 入口 | **PointNav + 视觉精修** | VLN 难以表达 0.1m 级精度要求 |
...
**Qwen-RobotNav 不能独立完成长距离导航，但它被设计成上层 Agent 可调用的局部导航原语。** 真正的长距离能力来自 Planner 把全局目标拆成局部子目标、完成坐标转换、并逐段调用 Qwen-RobotNav 执行。这种"薄执行器 + 厚规划器"的分工，正是它面向 Agentic 系统的核心设计哲学。
