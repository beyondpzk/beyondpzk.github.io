...
#### 5.7.7 Planner 深度解析：VLAC 闭环、模型选型与训练

以上是 Agentic Navigation System 的组件描述。下面从工程实现的角度拆解三个常被忽略的关键问题。

**Planner 和 Self-Reflector 是同一个 VLM 吗？**

是的。ABot-N0 采用的 **VLAC（Vision-Language-Action-Critic）**闭环机制，核心创新在于 **Planner 和 Critic 共用同一个 VLM**，通过 prompt 切换交替扮演两种角色：

| 模式 | Prompt 前缀 | 输出 |
|------|-----------|------|
| Policy 模式 | "你是一个导航规划器，请根据用户需求分解任务..." | 子任务序列 `G` |
| Critic 模式 | "你是一个任务评估器，判断当前步骤是否完成..." | `(r, f)` |

VLAC 的技术实现基于一个**通用奖励模型（General Reward Model）**。该模型在**大规模异构数据集**上训练，给定成对观测数据和语言目标，输出密集的进度增量和完成信号，无需任务特定的奖励函数设计，并支持**零样本上下文迁移**至未见过的任务和环境。

通过提示词控制，单个 VLAC 模型可交替生成奖励和动作标记，实现评论器与策略的统一。这本质上把传统 VLA（Vision-Language-Action）升级为 **VLA + Critic** 的四元闭环。

**Planner 需要单独训练吗？Planner 和 ABot-N0 是同一个模型吗？**

需要，但不是独立的。Planner 和 ABot-N0 是**两个独立的模型**：

| 组件 | 模型 | 定位 | 部署位置 | 频率 |
|------|------|------|---------|------|
| **Agentic Planner + Self-Reflector** | 独立 VLM（通用奖励模型） | 任务拆解 + 反思纠错 | 云端 RTX 4090 | 事件触发 |
| **ABot-N0 Actor** | Qwen3-4B + Flow Matching | 视觉感知 + 轨迹生成 | 边缘 Jetson Orin NX | 2Hz |

Planner **不是**直接拿一个现成 VLM 做 zero-shot 推理。它在大规模异构数据上训练为通用奖励模型（General Reward Model），训练目标是学会判断"当前状态离目标有多远、需要往哪个方向调整"。正是这个能力让它同时拥有任务拆解（Policy 模式）和完成评估（Critic 模式）。

框架上，ABot-Claw 基于 **OpenClaw**（高德开源的通用具身 agent 运行时），将最初为软件操作任务设计的运行时扩展为真实世界具身智能调度引擎。OpenClaw 提供执行循环、工具调用、记忆管理等编排基础设施，而 Planner VLM 充当其中最核心的决策引擎。

**Topo-Memory 是静态地图还是动态更新的？**

动态。Topo-Memory 不是一次性建好再用的静态地图，而是持续演化的世界模型。每次子任务执行完毕——无论成功还是失败——Self-Reflector 的输出都会写回 Topo-Memory：

```
g_i 执行完成 → Self-Reflector 评估 → 写回 Topo-Memory

成功案例: 在零食货架找到可乐
  → Object/POI Layer 写入: 零食货架 | 可乐 | 可达性=Confirmed

失败案例: 在零食货架没找到可乐
  → Object/POI Layer 写入: 零食货架 | 可乐 | 可达性=Excluded
  → 下次 Planner 查询时自动跳过
```

这种"成功和失败都写回记忆"的机制，是数据飞轮在产品侧最直接的形态。机器狗每次执行任务，走过的路线、发现的物体、失败的尝试，都能持续更新 Topo-Memory，让后续规划越来越精准。

**一个完整例子走一遍**

以论文中的典型案例"我要一罐可乐"为例，追踪完整的 Planner-Actor-Reflector 循环：

```
Step 1: Planner (VLM, CoT, Policy 模式)
  输入: I = "我要一罐可乐", O_t = 当前场景, M^L = Topo-Memory
  CoT思考: "用户需要饮料。M^L 显示附近有零食货架(坐标A)和售货机(坐标B)。
           先去零食货架，如果没找到再转向售货机。"
  输出: g₁ = Point-Goal → 到达坐标A
        g₂ = Object-Goal → 在零食货架搜索"可乐"

Step 2: Actor 执行 g₁, g₂
  ABot-N0 (2Hz) → Neural Controller (10Hz)
  → 到达零食货架，开始搜索

Step 3: Self-Reflector (VLM, Critic 模式)
  观测: "货架上有薯片、矿泉水、饼干。没有可乐。"
  输出: (r=False, f="零食货架上可乐不存在，建议尝试自动售货机")
  Topo-Memory 更新: 零食货架.可乐 = Excluded

Step 4: Planner 重规划（带上失败反馈 f）
  输入: I + M^L(已更新) + f
  CoT思考: "零食货架已排除。M^L 显示售货机在坐标B。转向售货机。"
  输出: g₃ = Point-Goal → 到达坐标B
        g₄ = Object-Goal → 在售货机搜索"可乐"

Step 5: Actor 执行 g₃, g₄ → 到达售货机，可乐在视野内

Step 6: Self-Reflector
  输出: (r=True, f="可乐在售货机中找到")
  Topo-Memory 更新: 售货机.可乐 = Confirmed

任务结束。全流程 6 步，2 次 Planner 调用，无人工介入。
```

值得注意的是 Step 4 中 Planner 重规划时，输入不只是失败反馈 `f`，还有**已经被 Step 3 更新过的 Topo-Memory**——零食货架的可乐状态已经是 `Excluded`。这意味着重规划不仅基于"这次失败了"，还基于"上次失败后的世界模型更新"，避免了重复犯同一个错误。

---

#### 5.7.8 长距离导航：配合高德地图实现跨城市级导航

以上讨论了任务分解与错误恢复。但一个常被忽略的问题是：**ABot-N0 能直接完成长距离导航任务吗，比如配合高德导航到达 6 公里外的一家咖啡店？**

**ABot-N0 本身不能。** ABot-N0 是一个局部导航执行器——它接收当前视觉观测，输出 5 个局部 BEV waypoint，有效视距约 5-15 米。它从不消费全局地图、GPS 坐标或路由信息。

但完整系统能。长距离导航的能力来自 **Agentic Planner + 高德地图 + Topo-Memory** 三层配合：

```
"去 6 公里外星巴克"
        ↓
Agentic Planner (云端 VLM)
  ├─ 调用高德全局路由: 6km 室外路径 → 分段 waypoint 序列
  ├─ 查询 Topo-Memory: 商场内星巴克在 B1 层，入口坐标已知
  └─ CoT 粗到细分解:

  室外段:
    g₁: Point-Goal → 沿人行道走到第一个路口
    g₂: Point-Goal → 过斑马线到对面
    ...（持续跟随高德路由分段）
  ...
  室内段:
    gₖ₊₁: Object/POI-Goal → 找到 B1 层星巴克
```

每段 g_i 交由 ABot-N0 在 5-15 米视距内逐段执行，反馈回流后继续下一段。论文中明确提到系统采用 "One Map" 策略统一室内外空间表示，整合 AMAP 全局路由与实时视觉决策，可执行"离开家→出小区→过街→到达 3 公里外商场内某餐厅"这类多阶段任务。6 公里和 3 公里在系统层面没有本质区别——都是 Planner 分段、ABot-N0 逐段执行的模式。

**室外段用的是什么能力？**

Point-Goal——和室内 Point-Goal 在任务定义上没有区别。Planner 把高德路由分段后，每一步都是"走到前方 10 米这个坐标点"，ABot-N0 接收到的输入格式完全一致。

但室内外 Point-Goal 的行为表现需要有本质差异：室外不能走到马路中间、必须走人行道、过街要等斑马线。这些行为约束不是通过切换任务模式实现的，而是 ABot-N0 的三阶段训练中 **SAFE-GRPO（Socially-Aware Flow Exploration GRPO）** 后训练注入的。Phase 3 在 SocCity 虚拟城市场景中用强化学习让模型学会区分"物理可通行 ≠ 社会可通行"：

| 指标 | 无 SAFE-GRPO (CityWalker) | 有 SAFE-GRPO (ABot-N0) |
|------|:--:|:--:|
| 闭环成功率 SR | 47.8% | **88.3%** |
| 距离合规率 DCR | 36.1% | **85.1%** |
| 时间合规率 TCR | 36.6% | **85.4%** |

同一模型、同一接口、同一训练权重，在室内走廊走在中间是正常的，在室外自动倾向于沿人行道内侧走——因为 SocCity 的训练数据让模型见过两种环境并学会了不同的行为规范。

**高德 API waypoint 能直接作为 Point-Goal 坐标吗？**

不能。高德 API 返回的 waypoint 精度在米级（通常 1-5 米偏差），且设计目标是为人或车规划路线——它标注的是路口中心点，不区分人行道、马路和隔离带。直接喂给 ABot-N0 做 Point-Goal 会导致机器狗走向马路中间而不是人行道。

正确做法是在 Planner 内部加入一层**局部目标精修**：

```
高德 waypoint (路口中心, 米级精度)
           │
    [语义意图 + 粗方向]
           │
    ┌──────▼─────────────────────┐
...
Planner 在这一步做的事不是"查地图输出坐标"，而是**视觉 grounding + 语义判断**：理解当前画面里的人行道边界和可通行区域，输出一个在当前视觉画面里可见、可达、且符合社会规范的局部目标。高德的 waypoint 只提供方向意图和距离大尺度，不提供精确的可执行目标。



> **更好的方案：语义指令而非坐标精修。** 既然高德 API waypoint 精度先天不足，更自然的做法是把 Planner 的这一层局部目标精修替换为**语义转换**——直接将高德的路由意图转成自然语言指令，用 ABot-N0 的 **VLN（Instruction-Following）** 模式执行。例如：
> 
> ```
> 高德路由: "往东北方向沿XX路走200m → 路口右转"
>     ↓ Planner 语义转换 ↓
> VLN 指令: "沿人行道往东北方向走，到第一个路口过斑马线右转，
>           再沿商铺外侧前行直到商场玻璃门前停下"
>     ↓
> ABot-N0 VLN 模式直接执行
> ```
...
#### 5.7.10 为什么视觉 ViT 只有 93M？是不是导航任务对视觉依赖不高？

ABot-N0 的端侧配置中，视觉骨干是 **93M SigLIP-B/16**，而 LLM backbone 是 **Qwen3-4B**。一个自然的问题是：为什么视觉模型这么小？是不是因为这些导航任务对视觉的依赖程度没有那么高？

##### 直接回答

**不是。** 导航任务对视觉的依赖程度其实很高，93M ViT 主要是**边缘部署的算力和内存限制**导致的，不是因为视觉不重要。

##### 导航任务的视觉依赖有多高？

看 ABot-N0 的五种任务，几乎每种都强依赖视觉：

| 任务 | 视觉依赖 |
|---|---|
| **Point-Goal** | 判断可通行区域、障碍物距离、地面几何 |
| **Object-Goal** | 识别物体类别，处理遮挡和视角变化 |
| **Instruction-Following** | 把语言路标（"红色沙发""黑色门垫"）ground 到图像 |
| **POI-Goal** | 识别商铺招牌、入口位置，精确到 0.1m 级控制 |
| **Person-Following** | 持续检测、重识别、预测动态目标运动 |

如果视觉不重要，ABot-N0 就不会设计：
- 全景/前视双模式；
- 显式的视觉历史记忆 `M^S`；
- 多相机输入和视角专属 token；
- Navigable Areas Analysis 这种视觉-几何联合推理任务。
...
1. **部署位置是 Jetson Orin NX**：16GB RAM，157 TOPS。这个硬件要同时跑 Qwen3-4B LLM、Flow Matching Action Expert、视觉编码器，视觉模型不能太大。
2. **"仅损失 3% 性能"**：说明 93M 的视觉表征已经足够，再加大带来的边际收益很小，但延迟会显著增加。

##### 为什么不用 "2B ViT + 2B LLM"？

有人可能会想：既然视觉重要，那把 ViT 也做到 2B，LLM 降到 2B，是不是更平衡？

这个方案对 ABot-N0 不 work，原因有三：

**1. 视觉 token 数量已经很多**

以全景模式为例：左、前、右三个相机，每个图像被 ViT 切成大量 patch token，三视角加起来可能有**几千个视觉 token**。

ViT 的计算大致是：

```
FLOPs_ViT ≈ num_image_tokens × d_model × num_layers
```

如果 ViT 从 93M 增加到 2B，参数量涨约 20 倍，视觉 token 端的计算也会近似涨 20 倍。这会直接把 2Hz 实时推理压垮。

**2. LLM 推理能力不能砍**

ABot-N0 的 Brain 需要完成五种任务的语义理解、长程指令解析、视觉历史整合、Reasoning Head 与 Action Head 的双头条件化。Qwen3-4B 已经是偏小的选择，再降到 2B 会明显削弱多任务统一的能力。

**3. 总参数量会接近上限**

当前配置：4B LLM + 93M ViT ≈ 4.1B 参数，已经占满 Jetson Orin NX 的 16GB RAM。换成 2B+2B 后：
- LLM 能力降级；
- ViT 计算爆炸；
- 内存更紧张。
...
按**参数量**看，LLM 占绝对大头（4B vs 93M，约 97% vs 2%）。

但按**推理延迟**看，视觉编码往往占不小比例，原因是：
- LLM 虽然大，但处理的 token 数量相对较少（几百个文本/视觉 token + KV cache）；
- ViT 处理的 token 数量巨大（多张图像 → 几千个 patch token）；
- 视觉 token 没有 KV cache 复用，每次都要重新编码。

所以更准确的说法是：

> **ViT 的单位参数量"算力密度"更高，但 LLM 的总延迟通常还是更大。** ABot-N0 用 93M ViT，是为了在"视觉 token 已经很多"的前提下，控制视觉端的总计算量。

##### 93M ViT 为什么"够用"？

够用不等于不重要。导航任务需要的视觉表征偏**中层**：
- 可通行区域；
- 障碍物轮廓；
- 物体存在性；
- 路标匹配。

它不需要像通用 VLM 那样做**细粒度图文理解**（比如解析菜单小字、识别远处车牌）。高层语义推理可以交给 Qwen3-4B 完成，视觉模型只需要提供"正确的视觉 token"。

##### 与 Qwen-RobotNav 的对比

Qwen-RobotNav 选择了另一条路：
- 用原生 **Qwen3-VL**，不刻意压缩 ViT；
- 部署在 **Jetson Thor** 或云端，而不是 Orin NX。

这说明：如果硬件更强，视觉模型可以更大；但如果硬件是 Orin NX，就必须做 ABot-N0 这种极致压缩。

##### 一句话总结

**ABot-N0 用 93M ViT 不是因为视觉不重要，而是因为导航任务的视觉需求可以用较小的模型满足，同时要在 Jetson Orin NX 上跑 2Hz。** 这是一个工程权衡（efficiency-accuracy trade-off），不是任务本质决定的。
