...
- **性能够用**：ONNX Runtime 已经满足速度要求
- **不是 NVIDIA GPU**：TRT 是 NVIDIA 专属，其他硬件用不了
- **快速原型阶段**：先跑通流程，后面再优化
- **算子特殊**：自定义算子多，转 TRT 开发成本高
- **多硬件部署**：一套 ONNX 通吃 CPU/GPU/NPU

---

不转 TRT 的代价

在 NVIDIA GPU 上，**TRT 通常比 ONNX Runtime 快 1.5~4 倍**，显存占用也更低。但代价是增加了转换、调试、维护的开发成本。
...
| 场景 | 推荐方案 |
|------|---------|
| 快速验证、性能够用 | **直接 ONNX Runtime** |
| NVIDIA GPU + 追求极致性能 | **转 TensorRT** |
| 非 NVIDIA 硬件 | **对应硬件的推理引擎** |

**一句话**：够用就别折腾，ONNX Runtime 简单省事；性能不够再上 TRT。
